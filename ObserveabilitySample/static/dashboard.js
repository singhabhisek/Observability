        ...
        $('#search').click(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    //     let cpuData = data["builtin:host.cpu.usage"] || []; // Ensure we handle CPU usage metric
                    //     let timestamps = [...new Set(cpuData.map(entry => entry.timestamp))]; // Get unique timestamps
                    //     let servers = [...new Set(cpuData.map(entry => entry.server))]; // Get unique server names
                    ...
                    //             label: server,
                    //             data: timestamps.map(ts => {
                    //                 let entry = cpuData.find(e => e.timestamp === ts && e.server === server);
                    //                 return entry ? entry.value : null; // Handle missing data points
                    //             }),
                    //             backgroundColor: `hsl(${index * 40}, 70%, 50%)`
                    //         };
                    ...
                    //     let cpuValues = cpuData.map(entry => entry.value);

                    //     console.log("CPU Timestamps:", timestamps);
                    //     console.log("CPU Servers:", servers);
                    //     createChart("cpuUsageBar", "bar", timestamps, datasets, "CPU Usage (%)", getYAxisMax(cpuValues, 100));
                    // })
                    // .catch(error => console.error("Error processing metrics:", error));
